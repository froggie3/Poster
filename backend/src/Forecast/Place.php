...
{
    $errorMessage = fn(\Throwable $error) => ["exception" => $error->getMessage(), "placeId" => $place->placeId];
    $message = null;  // message to send

    $messageBuilder = new MessageBuilder();

    $embed = new Embed($place->discord);
    ...
    try {
        ...
        $place->logger->debug("fetched forecast", [
            "placeId" => $place->placeId,
        ]);

        $telopNumber = (int)$response->trf->forecast[0]->telop;

        $telop = getAssociatesFromTelop($place->pdo, $telopNumber);

        $message = createMessage($header, $messageBuilder, $embed, $response, $telop);
    ...
    // on failed
    $message = createMessageOnfailed($header, $messageBuilder, $embed);
 ...
 * @return MessageBuilder
 */
function createMessageOnfailed(
    ...
    $message = getMessagePartial($header, $messageBuilder)
        ->setContent(
            sprintf(
                "%s 時点の天気予報の取得に失敗しました",
                $dt->format("H:i")
...
function createMessage(MessageHeader $header, MessageBuilder $messageBuilder, Embed $embed, WeatherForecast $response, Telop $tp): MessageBuilder
{
    ...
    // 投稿日時にする？
    $dt = new DateTimeImmutable($response->trf->trfAtr->reportedDate);

    $thumbnailUrl = "$header->baseUrl/{$tp->telopFilename}";

    $forecast = $response->trf->forecast[0];

    $embed = getEmbedPartial($header, $embed)
